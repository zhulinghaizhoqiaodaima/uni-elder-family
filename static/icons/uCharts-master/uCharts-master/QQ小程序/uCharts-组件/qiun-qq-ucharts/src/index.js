...
Component({
  ...
  methods: {
    ...
    resizeHandler() {
      ...
      let chartdom = qq.createSelectorQuery().in(this)
        ...
        .boundingClientRect(data => {
          if (data.width > 0 && data.height > 0) {
            ...
            if (this.data.type2d === true) {
              const query = qq.createSelectorQuery().in(this)
              query
                .select('#' + cid)
              ...
              cfu.option[cid].context = qq.createCanvasContext(cid, this);
              if(cfu.instance[cid] && cfu.option[cid] && cfu.option[cid].update === true){
                this._updataUChart(cid)
              }else{
                setTimeout(()=>{
    ...
    _tap(e,move) {
      ...
      if (this.inScrollView === true) {
        ...
        if(move){
          ...
          this.emitMsg({name: 'getIndex', params: {type:"getIndex", event:{ x: e.touches[0].x, y: e.touches[0].y }, currentIndex: currentIndex, legendIndex: legendIndex, id: cid, opts: cfu.instance[cid].opts}});
        }
      }
    },
    _touchStart(e) {
